...
export class FormTecnologicoComponent {
  ...
  constructor(
    private refrencia: MatDialogRef<FormTecnologicoComponent>,
    @Inject(MAT_DIALOG_DATA) public data: any,
    private entidadLocalizacion: LocalizacionService,
    private entidadUsuario: UsuarioService,
    private entidadCategoria: CategoriaService,
    private entidadDependencia: DependenciaService,
    private entidadTecnologico: TecnologicoService,
    private entidadMarca: MarcaService,
    private entidadBloque: BloqueService,
    private entidadUbicacion: UbicacionService
  ) {
  ...
  cargarFormulario() {
    this.formulario = new FormGroup({
      ...
      tec_anio_ingreso: new FormControl(this.data ? this.data.tec_anio_ingreso : '', [Validators.required, Validators.pattern('^[0-9]*$')]),
